...
class ApplicationGrille(tk.Tk):
    def __init__(self) -> None:
        ...
        self.title("Менеджер скинов VRM — Сетка")
    ...
    def _construire_entete(self) -> None:
        ...
        self.btn_vue_liste = tk.Button(
            entete,
            text="☰ Список (заблокировано)" if configuration_verrouillee else "☰ Список",
        ...
        self.btn_options = tk.Button(
            entete,
            text="⚙ Настройки (заблокировано)" if options_verrouillees else "⚙ Настройки",
    ...
    def _construire_onglets(self) -> None:
        ...
        self.btn_onglet_tous = tk.Button(
            barre, text="ВСЕ", relief="flat", command=lambda: self.set_onglet("tous")
        )
        self.btn_onglet_tous.pack(side="left")

        self.btn_onglet_favoris = tk.Button(
            barre, text="ИЗБРАННОЕ", relief="flat", command=lambda: self.set_onglet("favoris")
        )
        ...
        tk.Button(
            barre, text="Поиск", command=lambda: self.set_recherche(self.recherche_bar.get().strip())
    ...
    def _construire_bas(self) -> None:
        ...
        self.label_selection = tk.Label(
            selection_bar, text="Нажмите на скин, чтобы выбрать его.", anchor="w"
        )
        self.label_selection.pack(side="left")

        self.btn_appliquer = tk.Button(
            selection_bar, text="Применить выбранный скин", state="disabled", command=self.on_appliquer
        )
        self.btn_appliquer.pack(side="right")

        self.btn_rafraichir = tk.Button(selection_bar, text="Обновить", command=self.rafraichir)
    ...
    def toggle_favori(self, skin_name: str) -> None:
        if skin_name in self.__favoris:
            ...
            self.status_message.set(f"Скин удалён из избранного: {skin_name}")
        else:
            self.__favoris.add(skin_name)
            self.status_message.set(f"Скин добавлен в избранное: {skin_name}")
    ...
    def rafraichir(self, conserver_selection: bool = True) -> None:
        ...
        if not skins:
            tk.Label(
                self.grid_frame,
                text="Нет скинов, соответствующих поиску / этой вкладке.",
                bg="#e0e0e0",
                fg="gray",
                pady=30,
    ...
    def _creer_carte(self, skin: Path, ligne: int, colonne: int, skin_applique: Optional[Path]) -> None:
        ...
        badge = tk.Label(
            zone_image, text="✓ применён", bg=COULEUR_APPLIQUE, fg="white", font=("TkDefaultFont", 8, "bold")
        )
    ...
    def _charger_vignette(self, skin: Path, image_label: tk.Label) -> None:
        ...
        if not preview_path.exists():
            ...
            image_label.config(image="", text="Создание...", fg="gray")
    ...
    def _afficher_vignette_depuis_fichier(self, skin: Path, preview_path: Path, image_label: tk.Label) -> None:
        try:
            ...
            image_label.config(text="Нет превью", fg="gray")
    ...
    def _actualiser_selection_bar(self) -> None:
        if self.selected_skin is None:
            self.label_selection.config(text="Нажмите на скин, чтобы выбрать его.")
            self.btn_appliquer.config(state="disabled")
            return

        taille_ko = self.selected_skin.stat().st_size / 1024
        self.label_selection.config(text=f"Выбрано: {self.selected_skin.name} ({taille_ko:.1f} КБ)")
    ...
    def mettre_en_evidence_skin_applique(self, skin_applique: Optional[Path]) -> None:
        if skin_applique is None:
            self.current_applied_label.config(text="Применённый скин: нет")
        else:
            self.current_applied_label.config(text=f"Применённый скин: {skin_applique.name}")

    def on_appliquer(self) -> None:
        if self.selected_skin is None:
            messagebox.showwarning("Скин не выбран", "Выберите скин в сетке.")
            return
        ...
        try:
            ...
            self.status_message.set(f"Применённый скин: {skin_path.name} скопирован в {destination}")
            self.rafraichir()
